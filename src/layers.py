...
class PrimaryCapsuleLayer(torch.nn.Module):
    ...
    def __init__(self, in_units, in_channels, num_units, capsule_dimensions):
        ...
        for i in range(self.num_units):
            # kenel_size and padding is used to controled output of out_channels
            # eg. if no padding, towrad the end out_channels may contains no values.
            #                         (sliding kernel over the in_channel + padding)
            #
            # (N,# feature, # in_channels) -> (N, # feature, # out_channels)
